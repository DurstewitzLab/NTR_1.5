...
%
%4.0:
%4.1: 256: If a trial has less than "n_epoch" is not used in multivariate analyses 
%__________________________________________________________________________
%I-PRELIMINARIES
    %
...
%II-CROSS-VALIDATION. Removing a set of trials in turn, compute the discriminant
    ...
    %II.1. Defining validation data blocks
        ...
        %a) Multiple-discriminating statistics
            ...
            cum_global_multi_stats=50.*ones(val_blocks,4);%100.*(   (n_epochs-1)/n_epochs).*ones(val_blocks,4);
            %
...
%script. Version 1.0 only shows a limited, basic display.nneus26
...
%III.1 Multi-class statistics
    ...
    %   x-valid. realisations) for O=1 (=Fisher discriminant operating on the original or delayed
    %   axes or linear kernel)
    %   Column 2: SEM for O=1
    %   Column 3: Percent of outlier vectors (averaged over all x-valid.) for O=1
    %   Column 4: SEM for O=1
    %   Column 5: Percent of divergent task-epoch trajectories (averaged over all x-valid.) for O=1
    %   Column 6: SEM for O=1
    %   Column 7: Percent of outlier task-epoch trajectories (averaged over all x-valid.) for O=1
    %   Column 8: SEM for O=1
    %
    if multi
...
%III.3 Returning only selected statistics. This can be trivially changed
    ...
    disp(['CE multi: ',num2str(report_stats.multiClass_err), '%; div. Trajec. multi ',num2str(report_stats.multiClass_diver),'%' ]); 
