...
%#516@ebb:  Control over probability zero's fro avoiding JS errors
%#471@ebb: Inicialization of JS
%#537-548, 563@ebb: Saving JS for each class also
...
for i=1:n_labels,
    ...
    for j=1:n_bins_per_class(i),
        ...
        if (j==1)||(current_class_times(j)==1+current_class_times(j-1))&&(trajec_pointer<=max_trajec_length)
...
for i=1:n_patt,
    ...
    if (i==1)||((c==labels(i-1))&&(trajec_pointer<=max_trajec_length))
        ...
        if (misscla_consec<1)||(trajec_pointer>=min_trajec_length),
...
%     warning('class_trajec:wrongClassific','Unsuccesful classification. Returning over-chance values'),
% end
%
...
class_j_s=zeros(1,n_labels);%Means of "valid" j_s per class
counter=zeros(1,n_labels);
...
JSs=-1.*ones(n_labels*(n_labels-1),1); k=0; %Number of different J-S divergences (see below)
%
for i=1:n_labels
    ...
    if length(current_data(~isnan(current_data(:,selected_dim)),selected_dim))>4,
        ...
        try
            ...
            [chi2_class(i,1),chi2_class(i,2)]=chi2gof(current_data(~isnan(current_data(:,selected_dim)),selected_dim));
            %Kolmogorov-Smirnov. Note, distributions has to be standarized
            %first.
            [ks_class(i,1),ks_class(i,2)]=kstest(current_data(~isnan(current_data(:,selected_dim)),selected_dim));
            %Lilliefors
            [lill_class(i,1),lill_class(i,2)]=lillietest(current_data(~isnan(current_data(:,selected_dim)),selected_dim));
            %
        catch
    ...
    for j=1:n_labels
        ...
        class1data=current_data(~isnan(current_data(:,selected_dim)),selected_dim);
        class2data=current_data_bis(~isnan(current_data_bis(:,selected_dim)),selected_dim);
        %II.2.2 Univariate class-paiwise comparison tests
        try
            [ttest_class(i,j,1),ttest_class(i,j,2)]=ttest2(class1data,class2data);
            [ranksum_class(i,j,1),ranksum_class(i,j,2)]=ranksum(class1data,class2data);
            [ks2_class(i,j,1),ks2_class(i,j,2)]=kstest2(class1data,class2data);
        catch
        ...
        pdf_i=cum_posterior_probs(i,:)'; pdf_j=cum_posterior_probs(j,:)';
            %Forget about strictly zero probabilities and equalize lengths.
            %This can distort the measure and it is not used so far
%             pdf_i=pdf_i(pdf_i>0);pdf_j=pdf_j(pdf_j>0);n_i=length(pdf_i); n_j=length(pdf_j); 
%             if n_i>n_j, n_i=n_j; else n_j=n_i; end; 
%             pdf_i=pdf_i(1:n_i); pdf_j=pdf_j(1:n_j);
%             bound=bound(1:n_i);
            %
            ...
            if i<j, 
                k=k+1; 
                if (    isnan( j_s(i,j))||(abs(  j_s(i,j))>100)|| ( j_s(i,j)<0))
                   if full_stats_disp>0
                    warning('class_trajec:NaN_divergence_error', [' Jensen-Shanon is not number please trace the comparison ( class i, class j)=',num2str(i),',',num2str(j)]);
                    ...
                    %Sum per class
                    class_j_s(i)=class_j_s(i)+j_s(i,j);counter(i)=counter(i)+1;
                end
...
basic_stats.class_js=class_j_s./counter;
...
%
if full_stats_disp>2%Bug in previous version
    ...
    disp('     Wilcox-ranksum test h='),disp(num2str(ranksum_class(:,:,2))),disp('  p='),disp(num2str(ranksum_class(:,:,1))),
